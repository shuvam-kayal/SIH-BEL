...
export function freshAuthRequired(operation: string): boolean {
  ...
  return process.env.BEL_ENV === "production" && [
    "WALLET_ACTIVATE", "WALLET_REVOKE", "ROLE_ASSIGN", "GRANT_CREATE", "GRANT_REVOKE", "ASSET_TRANSFER", "JOB_VERIFY", "VALIDATOR_ADD", "VALIDATOR_REMOVE", "VALIDATOR_RESTORE", "VALIDATOR_REMOVE_CANCEL",
  ].includes(operation);
}
