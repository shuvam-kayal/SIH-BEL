...
import type { ValidatorAddInput, ValidatorRemovalInput, ValidatorRestoreInput } from "../../../shared/api";
...
export class ValidatorServiceImpl implements ValidatorService {
  ...
  async removeValidator(actorId: string, id: string, input: ValidatorRemovalInput): Promise<ValidatorRegistration> {
    ...
    const registration = await this.get(id); if (registration.status !== "ACTIVE") throw new ConflictError("Validator is not removable in its current state");
    ...
    registration.status = "REMOVED"; registration.removalHeight = input.removalHeight; registration.removalReason = input.reason.trim(); registration.txHash = result.transactionHash ?? result.txId; registration.blockNumber = result.blockNumber ?? null;
    await this.repositories.validators.save(registration); await this.recordHistory(registration, actor, "VALIDATOR_REMOVE", "ACTIVE", "REMOVED", input.reason.trim(), result); await this.notifications.validatorChanged("VALIDATOR_REMOVE", registration, actor);
    return { ...registration };
  }
  ...
  async restoreValidator(actorId: string, id: string, input: ValidatorRestoreInput): Promise<ValidatorRegistration> {
    ...
    const registration = await this.get(id); if (registration.status !== "REMOVED") throw new ConflictError("Only a removed validator can be restored");
  ...
  private async submit(type: "VALIDATOR_ADD" | "VALIDATOR_REMOVE" | "VALIDATOR_RESTORE", actor: Identity, registration: ValidatorRegistration, extra: Record<string, unknown>) {
    const user = await this.repositories.users.findByIdentityId(actor.identityId);
    try {
      const result = await this.chain.submitTransaction({ txId: randomUUID(), type, actorIdentity: actor.identityId || SYSTEM, actorWallet: user?.walletAddress ?? SYSTEM_WALLET, payload: { validatorId: registration.validatorId, nodeAddress: registration.nodeAddress, publicKey: registration.publicKey, signingPublicKey: registration.signingPublicKey, activationHeight: registration.activationHeight, ...extra }, timestamp: new Date().toISOString(), signature: "development" });
      const expected = type === "VALIDATOR_ADD" ? "ValidatorAdded" : type === "VALIDATOR_REMOVE" ? "ValidatorRemoved" : "ValidatorRestored";
